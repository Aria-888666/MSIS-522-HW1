...
""")
    ...
    if os.path.exists("models/random_forest.pkl"):
        ...
        # ── Extract the raw estimator and preprocessor from the pipeline ──
        # shap.Explainer cannot accept a sklearn Pipeline directly — we need to:
        # 1. Transform X through all steps EXCEPT the final estimator
        # 2. Pass the raw final estimator (RandomForestRegressor) to shap.Explainer
        from sklearn.pipeline import Pipeline

        def get_transformed_X_and_estimator(pipeline, X_raw):
            """
            Given a sklearn Pipeline, return:
              - X_transformed: numpy array after applying all preprocessing steps
              - final_estimator: the last step (the actual model)
              - feature_names: column names after transformation (if available)
            """
            if isinstance(pipeline, Pipeline):
                steps = pipeline.steps  # list of (name, transform) tuples
                X_t = X_raw.copy()
                for name, step in steps[:-1]:          # all steps except final
                    X_t = step.transform(X_t)
                final_est = steps[-1][1]               # last step = the model
                # Try to recover feature names from the preprocessor
                try:
                    feature_names = pipeline[:-1].get_feature_names_out()
                except Exception:
                    feature_names = [f"f{i}" for i in range(
                        X_t.shape[1] if hasattr(X_t, "shape") else len(X_t[0])
                    )]
                return X_t, final_est, feature_names
            else:
                # Not a pipeline — use as-is
                return X_raw.values if hasattr(X_raw, "values") else X_raw, pipeline, list(X_raw.columns)

        # Build a raw input sample (no manual encoding — let the pipeline handle it)
        X_full_raw = df[FEATURE_COLS].copy()
        X_sample_raw = X_full_raw.sample(200, random_state=42)

        with st.spinner("Computing SHAP values — this may take a moment..."):
            try:
                X_transformed, final_estimator, feat_names = get_transformed_X_and_estimator(
                    rf_pipeline, X_sample_raw
                )

                # Convert to DataFrame with feature names for nicer SHAP plots
                if not isinstance(X_transformed, pd.DataFrame):
                    X_transformed_df = pd.DataFrame(
                        X_transformed,
                        columns=feat_names[:X_transformed.shape[1]]
        ...
        if shap_ok:
            ...
            st.pyplot(fig)
            st.caption("""
**How to read this:** Each dot represents one observation. The x-axis shows the SHAP value (impact on
prediction). Color indicates the feature value (red = high, blue = low). Features are ranked by mean
absolute SHAP value — the most important features appear at the top. Wide spreads indicate that a
feature has highly variable impact across observations.
""")
            ...
            st.pyplot(fig)
            st.caption("""
**How to read this:** This bar chart ranks features by their average absolute SHAP value across all
200 sampled observations — a model-agnostic measure of global feature importance. Features at the top
have the largest average impact on predictions regardless of direction.
""")
...
""")
        ...
        if model_key == "random_forest" and os.path.exists("models/random_forest.pkl"):
            ...
            st.markdown("""
The waterfall plot below explains *this specific prediction* — it shows how each feature pushed
the prediction up (red, ↑) or down (blue, ↓) from the model's average baseline prediction.
This helps you understand exactly why the model gave this particular output for your chosen inputs.
""")
            try:
                from sklearn.pipeline import Pipeline as SKPipeline

                rf_pipeline_wf = joblib.load("models/random_forest.pkl")

                # Build raw template (pre-pipeline) with original dtypes
                X_template_raw = pd.DataFrame([{
                    col: df[col].mean() if col in NUM_COLS else df[col].mode()[0]
                    for col in FEATURE_COLS
                }])
                if "price" in X_template_raw.columns:
                    X_template_raw["price"] = price_val
                if "feat" in X_template_raw.columns:
                    X_template_raw["feat"] = float(feat_val)
                X_template_raw = X_template_raw[FEATURE_COLS]

                # Transform through pipeline preprocessing steps only
                if isinstance(rf_pipeline_wf, SKPipeline):
                    X_tmpl_t = X_template_raw.copy()
                    for name, step in rf_pipeline_wf.steps[:-1]:
                        X_tmpl_t = step.transform(X_tmpl_t)
                    final_est_wf = rf_pipeline_wf.steps[-1][1]
                    try:
                        feat_names_wf = rf_pipeline_wf[:-1].get_feature_names_out()
                    except Exception:
                        feat_names_wf = [f"f{i}" for i in range(X_tmpl_t.shape[1])]
                else:
                    X_tmpl_t = X_template_raw.values
                    final_est_wf = rf_pipeline_wf
                    feat_names_wf = FEATURE_COLS

                X_tmpl_df = pd.DataFrame(
                    X_tmpl_t,
                    columns=feat_names_wf[:X_tmpl_t.shape[1]]
                            if len(feat_names_wf) >= X_tmpl_t.shape[1]
                            else [f"f{i}" for i in range(X_tmpl_t.shape[1])]
                )

                explainer_wf = shap.TreeExplainer(final_est_wf)
                sv = explainer_wf(X_tmpl_df)
                ...
                st.warning(f"Could not generate waterfall plot: {e}")    ax.set_xlabel("Brand")
    ax.set_ylabel("logmove")
    st.pyplot(fig)
    st.caption("""
**What this shows:** There are meaningful differences in median sales across brands. The spread of the
...
""")
    ...
    if os.path.exists("models/random_forest.pkl"):
        ...
        # Prepare features — handle categorical encoding to match training
...
""")
        ...
        if model_key == "random_forest" and os.path.exists("models/random_forest.pkl"):
            ...
            st.markdown("""
The waterfall plot below explains *this specific prediction* — it shows how each feature pushed
the prediction up (red) or down (blue) from the model's baseline expected value.
""")
            try:
                rf_model = joblib.load("models/random_forest.pkl")
                X_bg = df[FEATURE_COLS].copy()
                for col in CAT_COLS:
                    X_bg[col] = X_bg[col].astype("category").cat.codes
                X_bg = X_bg.astype(float).sample(100, random_state=42)

                explainer_wf = shap.Explainer(rf_model, X_bg)
                sv = explainer_wf(X_template)
